...
\begin{document}
	...
	\begin{definition}[Factor of some Cotree]
		...
		\[\forall L\in \mathcal{L}(T_{min})\,\exists S \subseteq T_{min}:\quad L\in\mathcal{L}(S)\,\text{ and }\, S\cong F\qquad\qquad (1)\] 
	...
	\begin{theorem}
		...
		\begin{proof}
			...
			Let $S_i=(r_{S_i},\mathcal{C}_{S_i})$ then $|\mathcal{C}_{S_i}|\leq 2$ because it isn't the trivial factor and (2). For the reason that $T$ is minimal the children of $S_i$ as well, and because it there at least two $S_i$ itself has to be minimal.\\
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	We denote a given tree as balanced if it arises from an arbitrary minimal cotree put trough this algorithm. Written $\beta(T_{min})$.\\
	...
	\begin{theorem}
		For a minimal cotree $T_{min}$ and two cotree isomorphic subtrees $S_1,S_2\subseteq T_{min}$ with $\mathcal{L}(T_{min})\cap \mathcal{L}(S_1) \not = \emptyset \not = \mathcal{L}(T_{min})\cap \mathcal{L}(S_2)$
	\end{theorem}
